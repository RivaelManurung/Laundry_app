...
@section('header-title', 'Laporan Bulanan')
...
@section('content')
    <div class="mb-6 border-b border-gray-200">
        <nav class="-mb-px flex space-x-8">
            ...
            <a href="{{ route('reports.weekly') }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Mingguan</a>
            <a href="{{ route('reports.monthly') }}" class="border-indigo-500 text-indigo-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm">Bulanan</a>
        </nav>
    </div>

    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <form action="{{ route('reports.monthly') }}" method="GET" class="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-white p-4 rounded-lg shadow-sm border border-gray-100 w-full sm:w-auto">
            <div class="flex items-center gap-2">
                <label for="month" class="text-sm font-medium text-gray-700">Bulan:</label>
                <select name="month" id="month" class="border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
            ...
            <div class="flex items-center gap-2">
                ...
                <select name="year" id="year" class="border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
        ...
        <div class="flex gap-2">
            <a href="{{ route('reports.export', ['type' => 'monthly', 'month' => $month, 'year' => $year, 'format' => 'pdf']) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
            ...
            <a href="{{ route('reports.export', ['type' => 'monthly', 'month' => $month, 'year' => $year, 'format' => 'excel']) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-4 text-center text-gray-500">Tidak ada transaksi pada bulan ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
